from distutils.command.config import config
from urllib import response
from fastapi import APIRouter, Response
from config.db import conn
from schemas.user import userEntity,usersEntity
from models.user import User
from passlib.hash import sun_md5_crypt
from bson import ObjectId
from starlette.status import HTTP_422_UNPROCESSABLE_ENTITY


user = APIRouter()

@user.get('/users/',response_model=list[User],tags=["User"])
def search_users():
    """
    It returns a list of users from the database
    :return: A list of users
    """
    return usersEntity(conn.local.user.find())

@user.get('/user/{id}',response_model= User,tags=["User"])
def search_user(id: str):
    """
    It takes a string as an argument, converts it to an ObjectId, and then searches the database for a
    user with that id
    
    :param id: str
    :type id: str
    :return: A userEntity object
    """
    return userEntity(conn.local.user.find_one({'_id':ObjectId(id)}))


@user.post('/user/',response_model= User,tags=["User"])
def create_user(user:User):
    """
    It takes a user object, creates a new user object, encrypts the password, deletes the id, inserts
    the new user object into the database, and returns the new user object
    
    :param user: User
    :type user: User
    :return: A userEntity object
    """
    new_user = dict(user)
    new_user['password'] = sun_md5_crypt.encrypt(new_user['password'])
    del new_user['id']
    id = conn.local.user.insert_one(new_user).inserted_id
    final_user = conn.local.user.find_one({"_id":id})
    return userEntity(final_user)


@user.put('/user/{id}',response_model= User,tags=["User"])
def update_user(id: str, user:User):
    """
    Update_user(id: str, user:User)
    
    :param id: str
    :type id: str
    :param user: User
    :type user: User
    :return: A userEntity object
    """
    new_user = dict(user)
    old_user = conn.local.user.find_one({"_id":ObjectId(id)})
    '''sun_md5_crypt.verify (sin encriptar,encriptada) 
    para verificar contraseña'''
    new_user['password'] = old_user['password'] if len(new_user['password']) <= 0 else sun_md5_crypt.encrypt(new_user['password'])
    conn.local.user.find_one_and_update({'_id':ObjectId(id)}, {'$set':new_user})
    return userEntity(conn.local.user.find_one({"_id":ObjectId(id)}))

@user.delete('/user/{id}',status_code=HTTP_422_UNPROCESSABLE_ENTITY,tags=["User"])
def delete_user(id: str):
    """
    It deletes a user from the database and returns a 422 error code
    
    :param id: str
    :type id: str
    :return: A Response object with a status code of 422.
    """
    userEntity(conn.local.user.find_one_and_delete({'_id':ObjectId(id)}))
    return Response(status_code=HTTP_422_UNPROCESSABLE_ENTITY)

